...
if __name__ == "__main__":
    prs = argparse.ArgumentParser(
        formatter_class=argparse.ArgumentDefaultsHelpFormatter, description="""Q-Learning for Multi-Intersection"""
    )
    ...
    prs.add_argument(
        ...
        dest="route",
        type=str,
        default="single-intersection-vhvh.rou.xml",
        ...
        if args.fixed:
            ...
            while not done["__all__"]:
                ...
                for agent_id in ql_agents.keys():
                    ...
                    metrics["timestep"].append(env.sim_step)
                    metrics["phase"].append(env.traffic_signals[agent_id].time_since_last_phase_change)
                    metrics["queue_length"].append(env.traffic_signals[agent_id].get_total_queued())
                    metrics["waiting_time"].append(env.traffic_signals[agent_id].get_accumulated_waiting_time_per_lane)
                    metrics["green_time_used"].append(env.traffic_signals[agent_id].green_phase)
                    metrics["total_reward"].append(r[agent_id])
                    
                    total_reward += r[agent_id]

        print(out_csv)
        out_csv = 'sumo_simulation'
